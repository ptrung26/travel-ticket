...
import { MenuConfig, OraMenu, OraMenuItem } from '@app/routes/pages/cong-viec/menu.config';
...
@Component({
  templateUrl: './dashboard-cong-viec.component.html',
  styleUrls: ['./dashboard-cong-viec.component.scss'],
})
export class DashboardCongViecComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  isGrantedChild(premission: any[]): boolean {
    if (premission?.length == 0) {
      return false;
    }

    return premission.some((x) => this.isGranted(x.permission));
  }
