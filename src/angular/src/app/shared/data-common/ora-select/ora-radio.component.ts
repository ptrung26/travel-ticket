import { Component, OnInit, Optional, Inject, Input, Provider, forwardRef, EventEmitter, Output, OnDestroy } from '@angular/core';
...
export class OraRadioComponent implements OnInit, ControlValueAccessor, OnDestroy {
  ...
  constructor(@Optional() @Inject(SelectOptions) directive: ISelectOptions) {
    ...
    if (directive) {
      this.options$ = directive.options$;
      this._unSubcription.push(
        this.options$.pipe(debounceTime(300), distinctUntilChanged()).subscribe((result) => {
          this._option = result;
