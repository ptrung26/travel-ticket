...
@Directive({
    selector: '[dirLoaiHinhThaiDuLich]',
    providers: [
        {
            provide: SelectOptions,
            useExisting: LoaiHinhThaiDuLichComboDirective,
        },
    ],
})
export class LoaiHinhThaiDuLichComboDirective implements ISelectOptions, OnInit {
    ...
    getDataSourceFromServer() {
        this._dataService.getloaihinhthaidulichcombobox().pipe(
            map((lst) =>
                lst.map((item) => {
