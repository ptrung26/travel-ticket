...
@Directive({
    selector: '[dirLoaiHopDong]',
    providers: [
        {
            provide: SelectOptions,
            useExisting: LoaiHopDongComboDirective,
        },
    ],
})
export class LoaiHopDongComboDirective implements ISelectOptions, OnInit {
    ...
    getDataSourceFromServer() {
        this._dataService.getloaihopdongcombobox().pipe(
            map((lst) =>
                lst.map((item) => {
