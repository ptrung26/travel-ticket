...
@Directive({
    selector: '[dirLoaiKhach]',
    providers: [
        {
            provide: SelectOptions,
            useExisting: LoaiKhachComboDirective,
        },
    ],
})
export class LoaiKhachComboDirective implements ISelectOptions, OnInit {
    ...
    getDataSourceFromServer() {
        this._dataService.getloaikhach().pipe(
            map((lst) =>
                lst.map((item) => {
