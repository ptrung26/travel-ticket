...
@Component({
  ...
  template: `
    <div class="main-ora-date"  clickOutside (mouseenter)="mouseEnterMain()" (mouseleave)="mouseLeaveMain()" (clickOutsideAndOverlay)="clickOutsideAndOverlay()">
      <input
        nz-input
...
export class OraDataPickerComponent implements OnInit, ControlValueAccessor, AfterViewInit, OnDestroy {
  ...
  @Input() set value(v: DateTime) {
    if (v) {
      this.control.setValue(v.toJSDate());
    } else {
      this.control.reset();
    }
  }
