...
const VALUE_ACCESSOR: Provider = {
  ...
  useExisting: forwardRef(() => OraDataPickerCustomComponent),
  multi: true,
};

@Component({
  selector: 'ora-date-picker-custom',
  template: `
    <div clickOutside (mouseenter)="mouseEnterMain()" (mouseleave)="mouseLeaveMain()" (clickOutsideAndOverlay)="clickOutsideAndOverlay()">
      ...
      <ord-overlay-content #picker>
        <nz-date-picker nzInline [nzDisabledDate]="disabledDate" nzFormat="dd/MM/yyyy" [formControl]="control"></nz-date-picker>
      </ord-overlay-content>
...
export class OraDataPickerCustomComponent implements OnInit, ControlValueAccessor, AfterViewInit, OnDestroy {
  ...
  private obsInput() {
    this.subcriptionInput = this._inputValue.valueChanges.pipe(debounceTime(200), distinctUntilChanged()).subscribe((result) => {
      ...
      try {
        const arrStr = result ? result.split('/') : null;
